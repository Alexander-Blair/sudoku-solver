...
class SolverState
  class << self
    ...
    def solve(state)
      ...
      literal_id, negation = state.next_literal_id_and_negation || return

      new_state = state.deep_clone

      solve(state) if state.assert(literal_id)
      solve(new_state) if new_state.assert(negation)
    end
  end
  ...
  def next_literal_id_and_negation
    @guesses += 1
    literal_id = nil
    ...
    [literal_id, -literal_id] if literal_id
  end
